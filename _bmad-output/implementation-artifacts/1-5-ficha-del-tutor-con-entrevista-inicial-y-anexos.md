...
- 2026-06-09: Verificación manual completa (Playwright driveado + screenshots): flujo OK incl. lock optimista en ambas pestañas, alerta en vivo, upsert de anexos y recuperación post-conflicto. 2 fixes derivados: (1) `global-setup.ts` limpia las fichas `Tutor E2E %`/`Tutor Verif %` de corridas anteriores (la BD acumulaba basura de test al no haber DELETE de negocio); (2) `form-tutor.tsx` con `noValidate` + safeParse Zod para que los mensajes de validación salgan en español (la validación nativa del navegador depende del locale). Anotados como mejoras futuras: aviso "medio PDF sin archivo adjunto" y formato chileno de la fecha del anexo.
